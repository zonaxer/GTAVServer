...
ESX.RegisterServerCallback('esx_advancedgarage:storeVehicle', function (source, cb, vehicleProps)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local KickPossibleCheaters = true
	local UseCustomKickMessage = true
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND @plate = plate', {
		...
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				...
				if KickPossibleCheaters == true then
					if UseCustomKickMessage == true then
...
AddEventHandler('esx_advancedgarage:payhealth', function(price)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeMoney(price)
	TriggerClientEvent('esx:showNotification', source, 'Has pagado' .. price)
end)
